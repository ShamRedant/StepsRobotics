...
export default function TestimonialsSection() {
  ...
  const StarRating = ({ rating, center }) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span
          key={i}
          className={i <= Math.floor(rating) ? "opacity-100" : "opacity-50"}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill={center ? "#000" : "#FACC15"}
            viewBox="0 0 24 24"
            width="20"
            height="20"
          >
  ...
  const TestimonialCard = ({ testimonial, isCenter, onClick }) => (
    ...
    >
      ...
      <div
        className={`mx-auto pt-[10px] mb-5 rounded-[12px] overflow-hidden ${
          isCenter ? "border-white" : "border-gray-50"
        }`}
      >
        <Image
      ...
      <p
        className={`text-center text-sm ${
          isCenter ? "text-gray-800 font-medium" : "text-gray-600"
        }`}
      >
        {testimonial?.role}
      </p>
      ...
      <div className="max-w-7xl mx-auto container-custom">
        <div className="mb-4 mt-12">
          <h2 className="text-3xl sm:text-3xl font-[500] lg:text-4xl mb-4 leading-snug text-center sm:text-left text-font-orbitron text-yellow-400">
            STEPS Robotics{" "}
            <span className="text-black text-font-orbitron">Talks</span>
          </h2>
